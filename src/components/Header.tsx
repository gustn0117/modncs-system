      ...
      <nav className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            ...
            <Image src="/logo-icon.png" alt="모든CS시스템" width={44} height={44} className="h-10 w-auto" />
          ...
          <div className="hidden lg:flex items-center gap-1">
            {menuItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="relative px-4 py-2 text-gray-700 font-bold hover:text-navy-700 rounded-lg transition-all duration-200 text-[15px] group"
              >
                {item.name}
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-navy-600 rounded-full group-hover:w-2/3 transition-all duration-300" />
              </Link>
            ))}
            <Link
              href="/inquiry"
              className="ml-4 bg-navy-700 hover:bg-navy-800 text-white px-6 py-2.5 rounded-lg font-semibold hover:shadow-lg hover:shadow-navy-700/30 transition-all duration-300 text-sm"
            >
              무료 상담 신청
            </Link>
          </div>
          ...
          <div className="lg:hidden border-t border-gray-100 pb-4 animate-fadeIn">
            {menuItems.map((item) => (
              <Link
                ...
                className="block px-4 py-3 text-gray-700 hover:bg-navy-50 hover:text-navy-800 rounded-lg transition font-medium"
              >
                {item.name}
              </Link>
            ))}
            <Link
              href="/inquiry"
              onClick={() => setIsOpen(false)}
              className="block mx-4 mt-3 text-center bg-navy-700 text-white py-3 rounded-lg font-semibold hover:bg-navy-800 transition"
            >
              무료 상담 신청
            </Link>
          </div>
